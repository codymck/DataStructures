...
// PURPOSE: Set the visit number for a given vertex
// PARAMETERS: int - amount of visits
//             char - vertex being visited
void dgraph::visit(int visitNum, char vertex)
{
  int vIndex = int(vertex) - 65; // vIndex is the index of the given vertex as an integer

  Gtable[vIndex].visit = visitNum; // set the vertex visit number
}

// PURPOSE: check if a given vertex has been visited
// PARAMETERS: char - vertex being visited
bool dgraph::isMarked(char vertex)
{
  int vIndex = int(vertex) - 65; // vIndex is the index of the given vertex as an integer

  if (Gtable[vIndex].visit == 0) // if the visit number is 0, return false, else true
  {
    return false;
  }
  else
